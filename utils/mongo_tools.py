...
class MongoInterface():
    DEWH_DEFAULT_FIELDS = ['Temp', 'Status']
    PM_DEFAULT_FIELDS = ['ForActiveEnergy_Tot', 'RevActiveEnergy_Tot', 'ActivePower_Tot', 'ApparentPower_Tot']

    def __init__(self, collection=None, database=None, host='localhost', port=27017):
        ...
        if isinstance(collection, Collection):
            ...
            if database not in self._client.list_database_names():
                log.info("Database: '%s', does not exist - creating new", database)
            self._database = self._client[database]

            if collection not in self._database.list_collection_names():
                log.info("Collection: '%s', does not exist in Database: '%s' - creating new", collection,
                      self._database.name)
            self._collection = self._database[collection]

            log.info("Created MongoInterface using Database: '%s', and Collection: '%s'",
    ...
    def get_many_dev_cursor(self, device_type, dev_ids, self_col: Collection = None, start_time: DateTime = None,
                            end_time: DateTime = None, limit=None, fields=None):

        if self_col is None:
            self_col = self.collection
        ...
        if device_type == 'dewh':
            str_id_list = ["GEY" + str(dev_id) for dev_id in dev_ids]
            if fields is None:
                fields = MongoInterface.DEWH_DEFAULT_FIELDS
        elif device_type == 'pm':
            str_id_list = ["PM" + str(dev_id) for dev_id in dev_ids]
            if fields is None:
                fields = MongoInterface.PM_DEFAULT_FIELDS
        else:
            raise ValueError('Device type does not exist or is not supported')
        ...
        cur = self_col.find(*query, cursor_type=CursorType.NON_TAILABLE).sort("TimeStamp", 1)
    ...
    def get_many_dev_raw_dataframe(self, device_type, dev_ids, self_col: Collection = None, start_time: DateTime = None,
                                   end_time: DateTime = None, limit=None, fields=None, cursor=None):
    ...
    def set_string_field_to_datetime(self, self_col: Collection = None, field_tag='TimeStamp'):
        ...
        cur = self_col.find({field_tag: {'$not': {'$type': "date"}}}, {field_tag: 1})
        ...
        if bulk_requests:
            result = self_col.bulk_write(bulk_requests).modified_count
        else:
            result = 0

        print("Modified TimeStamp type for {} documents".format(result))
        return result
    ...
    def set_string_field_to_float(self, self_col: Collection = None, field_tag='GEY1.Temp'):
        ...
        cur = self_col.find({field_tag: {'$not': {'$type': "double"}}}, {field_tag: 1})
        ...
        print("Modified field type for {} documents".format(result))
        return result

    def append_from_collection(self, from_col: Collection, self_col: Collection = None, limit=None, newest=True,
                               override=False):
        if self_col is None:
            self_col = self.collection
        ...
        if limit is None:
            cur = from_col.find({}, {'_id': 0}).sort('_id', order)
        else:
            cur = from_col.find({}, {'_id': 0}).sort('_id', order).limit(limit)
        ...
        try:
            result = self_col.bulk_write(requests)
    ...
    def clone_from_collection(self, from_col: Collection, self_col: Collection = None, limit=None, newest=True,
                              override=False):

        if (self_col is not None) and (self_col != self._collection):
            self.collection = self_col
            print("MongoInterface now using Collection: '{}'".format(self.collection.name))
        ...
        print("Cloned {} documents".format(num_cloned))
        return num_cloned
...
if __name__ == '__main__':
    ...
    mi = MongoInterface(collection='Kikuyu', database='site_data')
