...
paths = glob.glob(r'../data/dewh_dhw_demand_actual_scenarios_15Min_200Lpd_mean.pickle')
dfs = OrderedStructDict()
for path in paths:
    if path.endswith('pickle'):
        dfs[os.path.basename(path).split('.')[0]] = pd.read_pickle(path)

df = dfs['dewh_dhw_demand_actual_scenarios_15Min_200Lpd_mean']
...
df_act.plot(drawstyle="steps-post", ax=ax0, lw=lineWidthNorm, color='k')

widths = 60 * 60 * 1e9 / 2  # half and hour
xmin, xmax = ax0.get_xlim()
positions = np.linspace(0, 23 * 3600 * 1e9, 24)

capprops = dict(linestyle='-', linewidth=2, color='b')
...
legend_elements = [
    Line2D([0], [0], color='k', lw=lineWidthNorm, label=r'Example actual demand'),
