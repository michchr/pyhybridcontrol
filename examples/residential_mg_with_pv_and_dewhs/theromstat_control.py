...
class DewhTheromstatController(NonPredictiveController):
    ...
    def solve(self, k, x_k=None, omega_tilde_k=None, external_solve=None, solver=None, *args, **kwargs):
        ...
        T_h_max_sub_T_h_on = self.sim_model.param_struct.T_h_max_sub_T_h_on
        T_h_max_sub_T_h_off = self.sim_model.param_struct.T_h_max_sub_T_h_off
        T_h_max = self.sim_model.param_struct.T_h_max

        if T_h <= T_h_max - T_h_max_sub_T_h_on:
            u_k = 1
        elif T_h >= T_h_max - T_h_max_sub_T_h_off:
            u_k = 0
        elif u_k_neg1 == 1:
            u_k = 1
        else:
