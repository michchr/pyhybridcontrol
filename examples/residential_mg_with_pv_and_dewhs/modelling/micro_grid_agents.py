...
class DewhAgentMpc(MicroGridDeviceAgent):
    ...
    def __init__(self, device_type='dewh', device_id=None,
        ...
        sim_model = sim_model if sim_model is not ParNotSet else DewhModel(param_struct=param_struct, const_heat=False)
        control_model = control_model if control_model is not ParNotSet else DewhModel(param_struct=param_struct,
                                                                                       const_heat=True)
...
class PvAgentMpc(MicroGridDeviceAgent):
    def __init__(self, device_type='pv', device_id=None,
        ...
        sim_model = sim_model if sim_model is not ParNotSet else PvModel(param_struct=param_struct)
...
class ResDemandAgentMpc(MicroGridDeviceAgent):
    def __init__(self, device_type='resd', device_id=None,
        ...
        sim_model = sim_model if sim_model is not ParNotSet else ResDemandModel(param_struct=param_struct)
        control_model = control_model if control_model is not ParNotSet else sim_model
...
class GridAgentMpc(MicroGridAgentBase):
    ...
    def __init__(self, device_type='grid', device_id=None,
        ...
        sim_model = sim_model if sim_model is not ParNotSet else GridModel(param_struct=param_struct, num_devices=0)
        control_model = control_model if control_model is not ParNotSet else sim_model
    ...
    def build_grid(self, k, deterministic_or_struct=True):
        if self.control_model.num_devices != self.num_devices or self.sim_model.num_devices != self.num_devices:
            sim_model = GridModel(num_devices=self.num_devices, param_struct=self.sim_model.param_struct)
            control_model = GridModel(num_devices=self.num_devices, param_struct=self.control_model.param_struct)
...
if __name__ == '__main__':
    ...
    omega_pv_profile = pd.read_pickle(
        os.path.realpath(r'../examples/data/pv_supply_norm_1000w_15min.pickle')) / 1000

    omega_resd_profile = pd.read_pickle(
        os.path.realpath(r'../examples/data/res_demand_norm_1000w_15min.pickle')) / 1000
    ...
    # omega_scenarios_profile = pd.read_pickle(os.path.realpath(r'../examples/data/dewh_omega_profile_df.pickle'))
