...
from controllers.mpc_controller.mpc_components.variables import MpcVariables, MpcVariableStruct
...
_mpc_objective_weight_types = ['vector', 'matrix']
...
class MpcObjectiveWeightBase(MpcComponentsBase):
    ...
    def __init__(self, var_name, weight_N_tilde=None, weight_N_p=None, weight_f=None, mpc_controller=None,
...
class MpcVectorWeight(MpcObjectiveWeightBase):
    _weight_type = 'vector'
...
class MpcMatrixWeight(MpcObjectiveWeightBase):
    _weight_type = 'matrix'
...
_mpc_objective_atom_types = ['linear', 'quadratic', 'L1', 'L2', 'Linf']
...
class MpcObjectiveAtomBase(MpcComponentsBase):
    ...
    def __init__(self, var_name, weight=None, is_rate_atom=False,
        ...
        super(MpcObjectiveAtomBase, self).__init__(mpc_controller, N_p=N_p, N_tilde=N_tilde,
                                                   mld_numeric_k=mld_numeric_k, mld_numeric_tilde=mld_numeric_tilde)
        ...
        if weight is not ParNotSet:
            if isinstance(weight, MpcObjectiveWeightBase):
                ...
                raise TypeError(
                    ...
                    f"subclass of {MpcObjectiveWeightBase.__name__}")
    ...
    def apply_atom(self, variable: MpcVariableStruct, N_p=None, N_tilde=None,
    ...
    @staticmethod
    def _gen_rate_variable_from_variable(variable: MpcVariableStruct, var_dim, N_p, N_tilde):
        d_variable = MpcVariableStruct()
        d_var_N_tilde = variable.var_N_tilde - cvx.vstack(
            [variable.var_k_neg1, variable.var_N_tilde[:-var_dim]])  # v(k)-v(k-1)
        MpcVariables._set_var_using_var_N_tilde(variable=d_variable, var_dim=var_dim, var_N_tilde=d_var_N_tilde,
                                                N_p=N_p, N_tilde=N_tilde)

        return d_variable


class LinearObjectiveAtom(MpcObjectiveAtomBase):
    _atom_type = 'linear'
...
class QuadraticObjectiveAtom(MpcObjectiveAtomBase):
...
class L1ObjectiveAtom(MpcObjectiveAtomBase):
    _atom_type = 'L1'
...
class LInfObjectiveAtom(MpcObjectiveAtomBase):
    _atom_type = 'Linf'
...
_WEIGHT_TYPES_MAP = {
    'vector': MpcVectorWeight,
    'matrix': MpcMatrixWeight,
}

MpcAtomStruct = struct_prop_dict('MpcAtomStruct', sorted_repr=True)


class MpcObjectiveAtoms(MpcComponentsBase):
    _allowed_atom_types = _mpc_objective_atom_types
    ...
    def __init__(self, mpc_controller=None, objective_atoms_struct=None, N_p=ParNotSet, N_tilde=ParNotSet,
                 mld_numeric_k=ParNotSet, mld_numeric_tilde=ParNotSet, **kwargs):
        super(MpcObjectiveAtoms, self).__init__(mpc_controller, N_p=N_p, N_tilde=N_tilde,
                                                mld_numeric_k=mld_numeric_k, mld_numeric_tilde=mld_numeric_tilde)
        self.update(_disable_process_args=True, objective_atoms_struct=objective_atoms_struct, **kwargs)

    def _reset(self):
        super(MpcObjectiveAtoms, self)._reset()
    ...
    def _set_atom_from_string(self, update_atoms, string_in, value, N_p, N_tilde, mld_info_k):
        ...
        if var_name in self._var_names and post_fix in self._allowed_post_fix:
            ...
            atom_type: MpcObjectiveAtomBase = _ATOM_TYPES_MAP[atom_type_name]
            weight_type: MpcObjectiveWeightBase = _WEIGHT_TYPES_MAP[weight_type_name]
    ...
    def _set_atom(self, update_atoms, var_name,
                  atom_name, atom_type: MpcObjectiveAtomBase, is_rate_atom,
                  weight_type: MpcObjectiveWeightBase, weight_length_name, value):

        var_atoms = update_atoms.get(var_name)
        if not var_atoms:
            update_atoms[var_name] = var_atoms = MpcAtomStruct()
        ...
        elif isinstance(var_atom, MpcObjectiveAtomBase):
            if isinstance(var_atom.weight, MpcObjectiveWeightBase):
